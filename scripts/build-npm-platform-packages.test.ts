...
it.layer(NodeServices.layer)("build-npm-platform-packages", (it) => {
  it.effect("refuses a partial release unless --allow-missing is passed", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual((error as NpmPackagesArchivesMissingError).missing, ["win32-x64"]);
    }),
  );
  ...
  it.effect("builds platform packages and a launcher that execs the installed one", () =>
    Effect.gen(function* () {
      ...
      assert.include(unsupported.stderr, "win32-x64");
      assert.include(unsupported.stderr, "https://github.com/MTVaught/t3code/releases");
    }),
  );
});
