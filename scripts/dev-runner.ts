...
import { DEFAULT_TREHER_HOME_DIRECTORY_NAME } from "@t3tools/shared/statePaths";
...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), DEFAULT_TREHER_HOME_DIRECTORY_NAME),
);
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .t3-treher > ambient T3CODE_HOME) is resolved
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    // (gitignored) `.t3-treher` — see @t3tools/shared/devHome for why this must
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Treher data directory; runtime state is stored under userdata (equivalent to T3CODE_HOME). Inside a git worktree this defaults to that worktree's own .t3-treher so dev state stays off the shared home.",
    ),
    Flag.optional,
