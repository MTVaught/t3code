...
function ProviderEnvironmentSection(props: {
  ...
  /**
   * Variables the driver always reads as secrets (Bob's API key). They render
   * as a dedicated password field ahead of the free-form rows and are kept
   * out of the editable list so they cannot be renamed or made plain text.
   */
  ...
  return (
    ...
    >
      {rows.length > 0 || props.fixedSensitiveVariables?.length ? (
        <div className="mt-3 min-w-0 space-y-2 pb-2">
          {props.fixedSensitiveVariables?.map((fixedVariable) => {
            ...
            return (
              <div key={fixedVariable.name} className="min-w-0 space-y-1">
                <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                  <span className="w-full min-w-0 truncate font-mono text-xs text-foreground sm:w-44 sm:shrink-0">
                    {fixedVariable.label}
                  </span>
                ...
                <p className="text-xs text-muted-foreground">{fixedVariable.description}</p>
              </div>
            );
          })}
...
export function ProviderInstanceCard({
  ...
  const driverKind: ProviderDriverKind | null = isProviderDriverKind(instance.driver)
    ? instance.driver
    : null;
  // Providers without a model picker (Bob) have no models section to show.
