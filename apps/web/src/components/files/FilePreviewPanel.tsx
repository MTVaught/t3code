...
import { Code2, Eye, FolderTree, Globe2, RefreshCw } from "lucide-react";
...
function renderedToggleLabel(isDocument: boolean, rendered: boolean): string {
  if (isDocument) return rendered ? "Show source" : "Show rendered document";
...
export default function FilePreviewPanel({
  ...
  const canToggleRendered = attachment === undefined && (isDocument || isHtml);
  const rendered = isDocument ? renderDocument : renderBrowserFile;
        ...
        >
          ...
          {canToggleRendered ? (
            <Tooltip>
              <TooltipTrigger
                render={
                  <Toggle
                    ...
                    aria-label={renderedToggleLabel(isDocument, rendered)}
                    variant="ghost"
                    size="sm"
                  >
              ...
              />
              <TooltipPopup>{renderedToggleLabel(isDocument, rendered)}</TooltipPopup>
            </Tooltip>
          ) : null}
          {canOpenInBrowser ? (
            <Tooltip>
