...
describe("resolveClerkSignInProps", () => {
  ...
  it("omits the redirect override on packaged desktop", () => {
    expect(resolveClerkSignInProps("treher-t3code://app/#/settings/general", true)).toEqual({});
  });

  it("omits the redirect override on development desktop", () => {
    expect(resolveClerkSignInProps("treher-t3code-dev://app/#/settings/general", true)).toEqual({});
  });
});
