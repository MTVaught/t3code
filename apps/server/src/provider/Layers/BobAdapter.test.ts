...
it.layer(bobTestLayer)("BobAdapter", (it) => {
  it.effect("maps a stream-json turn to canonical runtime events", () =>
    Effect.gen(function* () {
      ...
      // and `maxTokens` is the tier's context window (premium = 1,048,576) since
      // bob never reports the window size in its stream output.
      ...
      if (usage?.type === "thread.token-usage.updated") {
        ...
        assert.equal(usage.payload.usage.maxTokens, 1_048_576);
