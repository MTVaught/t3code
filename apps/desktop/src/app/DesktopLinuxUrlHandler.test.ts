...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: "T3 Code (Treher Nightly)",
      execTarget: '/home/al ice/Apps/T3 "100%" $HOME\\x.AppImage',
      scheme: "treher-t3code",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=T3 Code (Treher Nightly)");
    ...
    assert.include(entry, "MimeType=x-scheme-handler/treher-t3code;");
  });
  ...
  it("carries structured context on registration errors", () => {
    ...
    assert.equal(
      writeError.message,
      "Failed to register the treher-t3code:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
    ...
    assert.equal(
      exitError.message,
      "Failed to register the treher-t3code:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/treher-t3code;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "t3code-url-handler.desktop", "x-scheme-handler/treher-t3code"],
        },
      ]);
    });
  });
