...
describe("DesktopAppIdentity", () => {
  it.effect("uses Treher's isolated userData path", () =>
    withIdentity(
      Effect.gen(function* () {
        const identity = yield* DesktopAppIdentity.DesktopAppIdentity;
        const userDataPath = yield* identity.resolveUserDataPath;

        assert.equal(userDataPath, "/Users/alice/Library/Application Support/t3code-treher");
      }),
    ),
  );
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["T3 Code (Treher)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "T3 Code (Treher)");
