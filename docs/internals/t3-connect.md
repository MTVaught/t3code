...
```text
treher-t3code-dev://app/
treher-t3code://app/
```

Local desktop development uses `treher-t3code-dev://app`, while packaged builds use
`treher-t3code://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `treher-t3code-dev://app`; the production Clerk instance should only
need `treher-t3code://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["treher-t3code://app"]}'
```
