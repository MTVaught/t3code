...
`.github/workflows/release.yml` builds macOS (`arm64`), Linux (`x64`), and Windows (`x64`)
desktop artifacts from a single `v*.*.*` tag and publishes one GitHub release. It auto-enables
signing only when platform credentials are present. macOS uses Developer ID signing and Apple
notarization; Windows uses Azure Trusted Signing.
