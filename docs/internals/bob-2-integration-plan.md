...
| API-key authentication  | Bob 2 headless execution accepts `BOB_API_KEY`, while GUI-launched apps cannot reliably inherit interactive shell profiles.                                                                              | Offer an optional password-masked `BOB_API_KEY` instance setting; configured values override the ambient environment, while blank values inherit it.                                               |
...
Defaults are explicit and safe. Emit a configured credential as `BOB_API_KEY`;
when the setting is blank, inherit Bob's current environment behavior.
