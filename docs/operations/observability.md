...
stdout/stderr at `~/.t3-treher/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.t3-treher/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.t3-treher/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.t3-treher/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.t3-treher/dev/logs/server.trace.ndjson`.
...
```bash
TRACE_FILE="${T3CODE_HOME:-$HOME/.t3-treher}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.t3-treher/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.t3-treher/dev/logs/server.trace.ndjson"
```
