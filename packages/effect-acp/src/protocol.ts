...
export const makeAcpPatchedProtocol = Effect.fn("makeAcpPatchedProtocol")(function* (
  ...
  const sendRequest = Effect.fn("sendRequest")(function* (method: string, payload: unknown) {
    ...
    yield* offerOutgoing({
      _tag: "Request",
      // Keep numeric ACP ids numeric at runtime. Effect's encoded RPC type is
      // narrower than the JSON-RPC wire format accepted by this adapter.
      id: requestId as unknown as string,
      tag: method,
      payload,
      headers: [],
