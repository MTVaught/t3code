...
import { DEFAULT_BOB_MODEL, ProviderOptionSelections } from "./model.ts";
...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Render every line of each changed file in the diff panel instead of
  // collapsing unmodified regions between hunks.
  diffExpandUnchanged: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
...
export const ServerSettings = Schema.Struct({
  ...
  // Overrides the shell launched in terminals. Empty falls back to the
  // platform default ($SHELL on macOS/Linux, pwsh.exe on Windows). Set this to
  // run a shell that is not a registered login shell — the value is exec'd
  // directly, so it does not need to appear in /etc/shells.
...
export const ClientSettingsPatch = Schema.Struct({
  ...
  diffExpandUnchanged: Schema.optionalKey(Schema.Boolean),
